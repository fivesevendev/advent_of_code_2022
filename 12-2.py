...
def numFind():
    ...
    originalUC = deepcopy(unchecked) #keeps unmoded list for using at start of the for loop

    for everyA in [aaa for aaa in originalUC if originalUC[aaa][HEIGHT] == 97]: #checks every "a" starting pos
        try:
            startPos = everyA
            distance = 0
            checked = {}
            unchecked = deepcopy(originalUC)
            unchecked[startPos][COST] = 0 #sets start pos cost to 0
            unchecked[startPos][HEIGHT] = 97 #sets starting height equal to ord("a")
            doneChecking = False
                        ...
                        if pot not in checked: #ensure we haven't checked them already
                            if pot in unchecked: #ensure they exist within the grid provided
                                if unchecked[pot][HEIGHT] <= unchecked[cPos][HEIGHT] + 1: #ensures move is at most 1 elev higher than current elev
                                    ...
                                    cost = distance #cost based only on distance already traveled
            ...
            while backTrack != startPos:
                ...
                steps += 1
            shortestAlt.append(steps) #adds steps from this looop to list
        except:
            pass #passes all key errors
    return min(shortestAlt) #returns shortest run that completed successfully
